...
> 项目第一阶段已于 2026 年 8 月完成。第一版已建立从 Vue 交互界面、REST API、MySQL/Redis 业务状态、EMQX/MQTT 设备通信到 AI 顾问和报告快照的完整业务闭环，前端已移除 mock 数据并接入真实后端接口。当前进入发布与第二阶段持续优化。
...
第一阶段围绕 Java 后端技术栈、物联网应用和 AI Agent 顾问三条主线完成可运行版本；第二阶段不再扩张核心业务范围，主要进行上线、稳定性、性能、安全和可观测性优化。
