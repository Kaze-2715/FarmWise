...
项目采用前后端分离的模块化单体架构：后端位于 `backend/`，前端位于 `frontend/`。后端已完成数据库基础、认证授权、用户与文件、地块、设备、种植计划、环境监测、智能灌溉、农事任务、RBAC 后台管理、AI 技术顾问和只读快照报告接口，以及 MQTT 设备通信与控制。异常预警已完成遥测事件驱动的自动判定、地块级去重、Redis 状态管理、自动恢复、关联任务提交后重试，以及查询、人工创建和人工处理接口，低频补偿扫描留待后续优化。架构、选型理由和淘汰项见 [后端架构说明](docs/backend-architecture.md)。
