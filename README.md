...
项目采用前后端分离的模块化单体架构：后端位于 `backend/`，前端位于 `frontend/farmwise-vue/`。后端已完成数据库基础、认证授权、用户与文件、地块、设备、种植计划、环境监测、智能灌溉、农事任务和 RBAC 后台管理接口，以及 MQTT 设备通信与控制。异常预警将在自动生成、去重和恢复规则确定后继续。架构、选型理由和淘汰项见 [后端架构说明](docs/backend-architecture.md)。
...
- [ ] 确定预警自动生成、去重、恢复和关联任务规则后，实现异常预警接口。
- [ ] 实现 AI 顾问和只读快照报告接口。
