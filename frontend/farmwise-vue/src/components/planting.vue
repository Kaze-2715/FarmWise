<template>
  <div>
          ...
          <div class="mb-6 rounded-xl border border-gray-100 bg-gray-50 p-4">
            <div class="mb-4 flex items-center justify-between">
              ...
              <button type="button" :disabled="availableMetrics.length === 0"
                ...
                :class="availableMetrics.length === 0
    ...
    <!-- 添加阈值规则弹窗 -->
    <div v-if="addThresholdModalVisible && selectedLandId"
      class="fixed inset-0 z-50 flex items-center justify-center bg-white/30 backdrop-blur-sm">
      <div class="mx-4 w-full max-w-lg overflow-hidden rounded-xl bg-white shadow-xl">
        <div class="flex items-center justify-between border-b border-gray-200 p-6">
          <div>
...
const deleteThreshold = (threshold) => {
  const metricLabel = sensorMetricLabels[threshold.metric] || threshold.metric;
  const confirmed = window.confirm(`确定要删除“${metricLabel}”阈值规则吗？`);
...
const submitEditThreshold = () => {
  ...
  const index = environmentThresholds.value.findIndex(threshold => threshold.landId === selectedThreshold.value.landId && threshold.metric === selectedThreshold.value.metric);
...
const submitAddThreshold = () => {
  ...
  const duplicated = environmentThresholds.value.some(threshold => threshold.landId === form.landId && threshold.metric === form.metric);
